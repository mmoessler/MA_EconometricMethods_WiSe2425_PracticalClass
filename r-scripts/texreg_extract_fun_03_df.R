...
# MM edited to include AIC/BIC
# -- extract.lm (stats) --------------------------------------------------------

extract.lm <- function(model, include.rsquared = FALSE, include.adjrs = FALSE,
                       include.nobs = TRUE, include.fstatistic = FALSE,
                       include.rmse = FALSE, include.aic = TRUE, include.bic = TRUE, ...) {
  
  # MM: extract fitting from lm object
  ssr <- sum(model$residuals^2)
  t <- length(model$residuals)
  ssr.t <- ssr/t
  log.ssr.t <- log(ssr.t)
  ...
  # # see "ur.df" function in "urca" package
  # AIC <- stats::AIC(model, k = 2)
  # BIC <- stats::AIC(model, k = log(length(model$residuals)))
